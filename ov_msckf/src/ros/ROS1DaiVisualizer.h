...
#include "../../../depthai_standalone/include/depthai_standalone/oakd_helper.h"
...
public:
   ...
   */
  ROS1DaiVisualizer(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<VioManager> app, std::shared_ptr<DaiCameraHandle> cam_ptr , std::shared_ptr<Simulator> sim = nullptr);
  ...
  void setup_dai_subscribers(std::shared_ptr<ov_core::YamlParser> parser);
  ...
  void setOdometryCallback(std::function<void(const nav_msgs::Odometry&)> func) { _on_odom_ready = func ; }
  /**
  ...
  void dai_callback_inertial(const sensor_msgs::Imu& msg) ; 

  /// Callback for monocular cameras information
  void dai_callback_monocular(const sensor_msgs::Image& msg0, int cam_id0);


  /// Callback for synchronized stereo camera information
  void dai_callback_stereo(const std::pair<sensor_msgs::Image,sensor_msgs::Image>& msgs, int cam_id0, int cam_id1);
...
protected:
  ...
  /// Depthai camera Handle to grab data without ros.
  std::shared_ptr<DaiCameraHandle> _cam_ptr ;
  std::function<void(const nav_msgs::Odometry&)> _on_odom_ready ;  
